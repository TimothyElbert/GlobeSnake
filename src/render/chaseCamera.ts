...
export class ChaseCamera {
  ...
  update(position: Vector3, heading: Vector3, dt: number, speedFactor = 1): void {
    ...
    const z = this.zoom;
    const follow = this.followAngle * (0.62 + z * 1.1);
    const height = this.height * (0.55 + z * 1.35);
    ...
    this.camera.position.copy(this.anchor).multiplyScalar(1 + height * rush);
    this.apply();
  }
  ...
  private apply(): void {
    const z = this.zoom;
    const height = this.height * (0.55 + z * 1.35);
    this.camera.position.copy(this.anchor).multiplyScalar(1 + height);
